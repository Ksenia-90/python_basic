# Задача_3 Реализовать программу работы с органическими клетками. Необходимо создать класс Клетка.
# В его конструкторе инициализировать параметр, соответствующий количеству клеток (целое число).
# В классе должны быть реализованы методы перегрузки арифметических операторов: сложение (__add__()),
# вычитание (__sub__()), умножение (__mul__()), деление (def __sub__(self):__()).
...
class Cell:
    ...
    def __str__(self):
        return f"Клетка состоит из {self.units} ячеек"

    def __add__(self, other):
        return Cell(self.units + other.units)

    def __sub__(self, other):
        units_sub = int(self.units - other.units)
        if units_sub <= 0:
            raise Exception("in first cell unit has more second cell")
        return Cell(units_sub)

    def __mul__(self, other):
        return Cell(int(self.units * other.units))
    ...
    @staticmethod
    def make_order(cell, cell_in_row):
        res = ""
        for i in range(int(cell.units)):
            res += "*"
            if (i + 1) % int(cell_in_row) == 0:
                res += "\n"
        return res
...
print(Cell.make_order(cell_2, 2))
